...
| `pick` | `x`, `y` | Diagnostic: report BOTH the renderer-accurate pick and the legacy AABB/sphere proxy pick at a screen point (`renderer_index`/`renderer_object`, `proxy_index`/`proxy_object`). Does NOT change selection. Proves the renderer pick is triangle-accurate where the proxy mis-picks a bounding box. |
