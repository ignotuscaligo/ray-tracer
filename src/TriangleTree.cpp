...
void printNode(const std::string& name, std::shared_ptr<TriangleTree::Node> node)
{
    if (node->left)
    {
...
std::shared_ptr<TriangleTree::Node> TriangleTree::root()
{
    return m_root;
}
...
float TriangleTree::getPivot(const Triangle& triangle, Axis axis)
{
    return triangle.center.getAxis(axis);
}

float TriangleTree::axisMedian(const std::vector<Triangle>& triangles, Axis axis)
{
    ...
    {
        return TriangleTree::getPivot(triangles[0], axis);
    }
...
void TriangleTree::castRayIntoNode(const Ray& ray, std::shared_ptr<TriangleTree::Node> node, std::vector<Hit>& hits)
{
...
std::shared_ptr<TriangleTree::Node> TriangleTree::generateTree(const std::vector<Triangle>& triangles, Axis axis)
{
    ...
    std::shared_ptr<TriangleTree::Node> node = std::make_shared<TriangleTree::Node>();
    node->axis = axis;
    ...
    if (triangles.size() <= 1)
    {
        node->page = std::make_unique<TriangleTree::Node::Page>();
    ...
    else
    {
        ...
        for (const auto& triangle : triangles)
        {
            float pivot = TriangleTree::getPivot(triangle, axis);
            if (pivot < node->pivot)
            {
